...
fprintf('n_elements  n_vertices   H1-error   rate    L2-error   rate    error_est \n');
...
while (1)
  ...
  if(iter_counter > 5) 
    p = polyfit(log(all_N_vert(end-4:end)),log(all_H1_err(end-4:end)),1);
    h1_rate = p(1);
    p = polyfit(log(all_N_vert(end-4:end)),log(all_L2_err(end-4:end)),1);
    l2_rate = p(1);
    figure(3)
    fprintf(' %8d    %8d  %10.7f  %5.2f  %10.7f  %5.2f  %10.7f  \n', ...
      mesh.n_elem, N_vert, h1, h1_rate, l2, l2_rate, est);
    loglog( ...
      all_N_vert,all_H1_err,'*-', ...
      all_N_vert,all_est,'o-', ...
